...
train_set = IEDataset(config.train_file, max_length=config.max_length, use_type=config.use_type)
...
print(f"Loading model from {config.pretrained_weight_path}")
state = torch.load(config.pretrained_weight_path, map_location=map_location)

#update vocabs using new dataset
old_vocabs = state['vocabs']
old_output_dim = len(old_vocabs['entity_label'])
vocabs = update_vocabs(old_vocabs, [train_set, dev_set, test_set])
# print(vocabs)
...
# load source model
source_model = StructuralModel(Config.from_dict(state['config']), state['vocabs'])
source_model.load_state_dict(state['model'])
source_model.cuda(device=config.gpu_device)
source_model.eval()


# create adapter model
#model = Adapter(config, vocabs, old_output_dim)
model = Adapter(config, vocabs, source_model.feature_dim)
# load pretrained parameter
model.load_pretrained_weight(state["model"])
...
for epoch in range(config.max_epoch):
    ...
    for batch_idx, batch in enumerate(DataLoader(
        ...
        # inference using source model
        _, prior_prob = source_model.predict(batch)
        source_model.eval()
        loss = model(batch, prior_prob, logger, 'train', current_step)
        current_step += 1
    ...
    for batch in DataLoader(dev_set, batch_size=config.eval_batch_size,
        ...
        _, prior_prob = source_model.predict(batch)
        source_model.eval()
        pred_entities, _ = model.predict(batch, prior_prob)
    ...
    if best_dev_role_model:    
        ...
        best_detail = details
        result = json.dumps(
            {'epoch': epoch, 'test': (precision, recall, f1)}
        )
        with open(log_file, 'a', encoding='utf-8') as w:
            w.write(result + '\n')
print(best_detail)
