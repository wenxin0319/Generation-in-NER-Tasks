...
class BaseInputFormat(ABC):
    ...
    def format_input(self, example: InputExample, multitask=False, task_descriptor=None, class_option=False):
        ...
        if int(os.environ['CLASS_OPTION']) == 1:
            random.shuffle(example.input_class_string)
            _string = ' [CLS-SEP] '.join(example.input_class_string)
            class_op = '[SEP] ' + _string + ' [SEP] '
            if int(os.environ['CLASS_FIRST']) == 1:
                res = class_op + res
            else:
                res = res + class_op
        return res
