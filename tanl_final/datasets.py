...
class JointERDataset(BaseDataset):
    ...
    def evaluate_example(self, example: InputExample, output_sentence: str, model=None, tokenizer=None) -> Counter:
        ...
        if int(os.environ['DEBUG']) == 1:
            with open("./log/output1.log","a+") as iq:
                iq.write("output_sentence: " + str(output_sentence) + "\n")
        predicted_entities, predicted_relations = res[:2]
        json_line = {}
        transfer = {"person":"PER","location":"LOC","miscellaneous":"MISC","organization":"ORG"}
        with open("output.json" ,"a+") as output_file:
            json_line["tokens"] = example.tokens
            for i in list(predicted_entities):
                r = list(i)
                json_line["pred object"] = [transfer[r[0]], r[1], r[2]]
        ...
        if int(os.environ['DEBUG']) == 1:
            with open("./log/gt_entities.log","a+") as write_file:
                write_file.write(str(gt_entities)+"\n")

            with open("./log/predicted_entities.log","a+") as write_file:
        ...
        if int(os.environ['DEBUG']) == 1:
            ...
            for i in predicted_entitiesl:
                if i in gt_entitiesl:
                    ...
                    with open("./log/correct.log","a+") as write_file:
                        write_file.write(str(i)+"\n")
            with open("./log/pred.log",'a+') as f:
                f.write(str(predicted_entitiesl)+ "\n") 
            with open("./log/gold.log",'a+') as f:
    ...
    def evaluate_dataset(self, data_args: DataTrainingArguments, model, device, batch_size: int, macro: bool = False) \
        ...
        if int(os.environ['DEBUG']) == 1:
            with open("./log/gold-answer.log","a+") as score_file:
...
class MultiWoz(BaseDataset):
    ...
    def compute_features(self, max_input_length: int, max_output_length: int, static_class_choice: List[str] = [], 
