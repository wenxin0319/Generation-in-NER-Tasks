...
best_model_path = os.path.join(output_dir, 'best_model.stage1.mdl')
dev_prediction_path = os.path.join(output_dir, 'pred.dev.stage1.json')
test_prediction_path = os.path.join(output_dir, 'pred.test.stage1.json')
...
# stage 1
# load data
train_set = NERCoNLL_Dataset(config.input_style, tokenizer, config.train_file1, config.used_class_stage1, config.max_token_length, config.max_input_length, config.max_output_length, config.class_dropout, no_bos, config.shuffle_class_order)
#dev_set = NERCoNLL_Dataset(config.input_style, tokenizer, config.dev_file1, config.test_class_options_stage1, config.max_token_length, config.max_input_length, config.max_output_length, 0.0, no_bos, config.shuffle_class_order)
dev_set = NERCoNLL_Dataset(config.input_style, tokenizer, config.dev_file1, config.used_class_stage1, config.max_token_length, config.max_input_length, config.max_output_length, config.class_dropout, no_bos, config.shuffle_class_order)
test_set = NERCoNLL_Dataset(config.input_style, tokenizer, config.test_file, config.test_class_options_final, config.max_token_length, config.max_input_length, config.max_output_length, 0.0, no_bos, config.shuffle_class_order)
...
for epoch in range(1, config.max_epoch+1):
    ...
    for batch_idx, batch in enumerate(DataLoader(dev_set, batch_size=config.eval_batch_size, 
        ...
        for bid, (i_text, g_text, p_text, gold, ori_tokens) in enumerate(zip(input_text, gold_text, pred_text, ground_truths, original_tokens)):
            #template = NERCoNLL_template(config.input_style, ori_tokens, config.test_class_options_stage1)
            template = NERCoNLL_template(config.input_style, ori_tokens, config.used_class_stage1)
    ...
    if best_dev_flag:    
        ...
        for batch_idx, batch in enumerate(DataLoader(test_set, batch_size=config.eval_batch_size, 
            ...
            original_tokens = batch.tokens
            for bid, (i_text, g_text, p_text, gold, ori_tokens) in enumerate(zip(input_text, gold_text, pred_text, ground_truths, original_tokens)):
                template = NERCoNLL_template(config.input_style, ori_tokens, config.test_class_options_final)
...
# stage 2
# load data
train_set = NERCoNLL_Dataset(config.input_style, tokenizer, config.train_file2, config.used_class_stage2, config.max_token_length, config.max_input_length, config.max_output_length, config.class_dropout, no_bos, config.shuffle_class_order)
#dev_set = NERCoNLL_Dataset(config.input_style, tokenizer, config.dev_file2, config.test_class_options_stage2, config.max_token_length, config.max_input_length, config.max_output_length, 0.0, no_bos, config.shuffle_class_order)
dev_set = NERCoNLL_Dataset(config.input_style, tokenizer, config.dev_file2, config.used_class_stage2, config.max_token_length, config.max_input_length, config.max_output_length, config.class_dropout, no_bos, config.shuffle_class_order)
test_set = NERCoNLL_Dataset(config.input_style, tokenizer, config.test_file, config.test_class_options_final, config.max_token_length, config.max_input_length, config.max_output_length, 0.0, no_bos, config.shuffle_class_order)
...
best_model_path = os.path.join(output_dir, 'best_model.stage2.mdl')
dev_prediction_path = os.path.join(output_dir, 'pred.dev.stage2.json')
test_prediction_path = os.path.join(output_dir, 'pred.test.stage2.json')
...
for epoch in range(1, config.max_epoch+1):
    ...
    for batch_idx, batch in enumerate(DataLoader(dev_set, batch_size=config.eval_batch_size, 
        ...
        original_tokens = batch.tokens
        for bid, (i_text, g_text, p_text, gold, ori_tokens) in enumerate(zip(input_text, gold_text, pred_text, ground_truths, original_tokens)):
            #template = NERCoNLL_template(config.input_style, ori_tokens, config.test_class_options_stage2)
            template = NERCoNLL_template(config.input_style, ori_tokens, config.used_class_stage2)
            # decode predictions
            pred_object = template.decode(p_text)
