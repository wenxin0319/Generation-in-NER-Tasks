...
class BaseDataset(Dataset, ABC):
    ...
    def __init__(
            ...
            add_class_choice: List[str],
            mapping_input_choice: List[str],
            mapping_output_choice: List[str],
        ...
        self.static_class_choice = static_class_choice
        self.add_class_choice = add_class_choice
        self.mapping_input_choice = mapping_input_choice
        self.mapping_output_choice = mapping_output_choice
        self.train_subset = train_subset
        self.train_episode_idx = train_episode_idx
        self.steps = steps
        ...
        with torch_distributed_zero_first(local_rank):
            ...
            if os.path.exists(cached_data_file) and not overwrite_cache:
                ...
                self.examples = self.load_data(mode=mode, episode_idx=self.train_episode_idx, subset=self.train_subset, seed=seed)
                ...
                self.features = self.compute_features(
                    ...
                    episode_idx=self.train_episode_idx,
                    steps=self.steps,
    ...
    # note that we only support the same subset and the same continula learning dividing method, 
    # otherwise it would be wierd on multidataset settings.
    def load_data(self, mode: str, episode_idx: int = 1, subset: float = 1.0, seed: int = None) -> List[InputExample]:
        """
        ...
        for split in splits:
            examples += self.load_data_single_split(split, episode_idx, subset, seed=seed)
    ...
    def compute_features(self, max_input_length: int, max_output_length: int, episode_idx: int, steps: int,
        static_class_choice: List[str] = [], add_class_choice: Dict = {}, multitask: bool = False):
        
        input_sentences = []
        output_sentences = []
        local_add_class_choice = add_class_choice
        steps=7
        print(episode_idx * steps)
        local_static_class_choice = list( static_class_choice[:episode_idx * steps] + list(local_add_class_choice.keys()))
        if os.environ["STRATEGY"] == "ADD":
            for example in self.examples:
                for few_shot_item, few_shot_item_number in zip(local_add_class_choice.keys(), local_add_class_choice.values()):
                    if few_shot_item in local_static_class_choice:
                            if few_shot_item_number <= 0:
                                local_static_class_choice.remove(str(few_shot_item))
                tmp_output_string = []
                tmp_input_string = []
                for label_index in local_static_class_choice:
                    if random.random() > float(os.environ["DROPOUT_PROB"]):
                        tmp_output_string.append(label_index)
                        tmp_input_string.append(label_index)
                example.output_string = tmp_output_string
                example.input_class_string = tmp_input_string
                tmp_example_entity = []
                for exampleInstance in example.entities:
                    if exampleInstance.type.natural in example.output_string:
                        for few_shot_item, _ in zip(local_add_class_choice.keys(), local_add_class_choice.values()):
                            if exampleInstance.type.natural == few_shot_item:
                                local_add_class_choice[few_shot_item] -= 1
                        tmp_example_entity.append(exampleInstance)
                example.entities = tmp_example_entity
                input_sentences.append(self.input_format.format_input(example, multitask=multitask))
                output_sentences.append(self.output_format.format_output(example))
        elif os.environ["STRATEGY"] == "HOLD":
            full_class_choice = ["person", "location", "organization", "vehicle", "geographical entity", "weapon", "facility"]
            for example in self.examples:
                tmp_input_string = []
                example.input_class_string = tmp_input_string
                for exampleInstance in example.entities:
                    if exampleInstance.type.natural not in example.output_string:
                        example.output_string.append(exampleInstance.type.natural)
                        example.input_string.append(exampleInstance.type.natural)
                for label_index in full_class_choice:
                    if random.random() > float(os.environ["ACCEPT_PROB"]):
                        if label_index not in example.output_string:
                            example.output_string.append(label_index)
                            example.input_string.append(label_index)
                input_sentences.append(self.input_format.format_input(example, multitask=multitask))
                output_sentences.append(self.output_format.format_output(example))
        elif os.environ["STRATEGY"] == "FEWSHOT":
            for example in self.examples:
                for few_shot_item, few_shot_item_number in zip(local_add_class_choice.keys(), local_add_class_choice.values()):
                    if few_shot_item in local_static_class_choice:
                        if few_shot_item_number <= 0:
                            local_static_class_choice.remove(str(few_shot_item))
                example.output_string = local_static_class_choice
                example.input_class_string = local_static_class_choice
                tmp_example_entity = []
                for exampleInstance in example.entities:
                    if exampleInstance.type.natural in example.output_string:
                        for few_shot_item, _ in zip(local_add_class_choice.keys(), local_add_class_choice.values()):
        ...
        if int(os.environ['DEBUG']) == 1:
            with open("./input.log","a+") as i1:
                for sentence in input_sentences:
                    i1.write(str(sentence) + "\n")
                
        if int(os.environ['DEBUG']) == 1:
            with open("./output.log","a+") as o1:
                for sentence in output_sentences:
                    o1.write(str(sentence) + "\n")
