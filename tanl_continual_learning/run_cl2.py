...
def main():
    ...
    parser.add_argument('-go', '--gold', action='store_true', default=True, help='print gold answers')
    ...
    SummaryWriter = tf.compat.v1.summary.FileWriter(training_args.tf_log)
    # tf.summary is deprecated so change to tf.compat.v1.summary
    ...
    model_episode_dir = ["" for _ in range(len(episode_indices))]
    ...
    for ep_idx in episode_indices:
        ...
        episode_output_dir += "0808-0714-add0.8drop0"
        ...
        model = None
        if model_episode_dir[ep_idx-1] != "":
            model_args.model_name_or_path = model_episode_dir[ep_idx-1]
        ...
        if training_args.do_train:
            ...
            for dataset_name in dataset_names:
                ...
                dataset = load_dataset(
                    ...
                    gold_answer_click=False,
                    train_episode_idx=ep_idx,
                )
                train_datasets.append(dataset)
            
            # Now we have the eval options and training options merged during training process to reduce the number of parameters
            train_dataset = torch.utils.data.ConcatDataset(train_datasets) if training_args.do_train else None
            eval_datasets_during_training = []
            if str(training_args.evaluation_strategy) == "EvaluationStrategy.EPOCH":
                # training_args.eval_dataset_during_train may have some underlying bugs when it is deleted
                if data_args.eval_dataset_during_train is not None:
                    eval_datasets_names = data_args.eval_dataset_during_train.split(",") 
                    for dataset_name in eval_datasets_names:
                        logging.info(f'Process dataset {dataset_name} (train)')
                        eval_dataset_during_training = load_dataset(
                            dataset_name, data_args, split=data_args.eval_split,
                            max_input_length=data_args.max_seq_length, max_output_length=data_args.max_output_seq_length,
                            static_class_choice= data_args.static_class_choice.split(","),
                            add_class_choice = json.loads(data_args.add_class_choice),
                            mapping_input_choice = json.loads(data_args.mapping_input_choice),
                        ...
                        eval_datasets_during_training.append(eval_dataset_during_training)
                else:
                    eval_datasets_during_training = train_datasets 
            else:
                eval_dataset = None

            # this has some potential bugs if you don't chooose evaluation strategy as epoch
            eval_dataset = torch.utils.data.ConcatDataset(eval_datasets_during_training) if (training_args.do_train and str(training_args.evaluation_strategy) == "EvaluationStrategy.EPOCH") else None
            ...
            model_episode_dir[ep_idx % int(data_args.episodes[-1])] = episode_output_dir
        ...
        if training_args.local_rank in [-1, 0] and (training_args.do_eval or training_args.do_predict):
            ...
            for comb in itertools.product(evaluation_splits, evaluation_dirs, eval_dataset_names):
                ...
                res = evaluate(
                    ...
                    gold_answer_click = args.gold, episode_idx = ep_idx
                )
                # store results
