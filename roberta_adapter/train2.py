...
import copy
import ipdb
from util import (Logger, generate_vocabs, update_vocabs)
...
best_role_model = os.path.join(output_dir, 'best.base.mdl')
...
train_set = IEDataset(os.path.join(config.train_folder, 'train+1.txt'), max_length=config.max_length, use_type=config.use_type)
print('==============Prepare Dev Set=================')
dev_set = IEDataset(os.path.join(config.dev_folder, 'dev+1.txt'), max_length=config.max_length, use_type=config.use_type)
...
for epoch in range(config.max_epoch):
    ...
    for batch_idx, batch in enumerate(DataLoader(
            ...
            shuffle=False, drop_last=True, collate_fn=train_set.collate_fn)):
...
with open(log_file, 'a', encoding='utf-8') as w:
    w.write('===============================\n')
    w.write('====Start Second Step==========\n')
    w.write('===============================\n')

# Step 2
# datasets
print('==============Prepare Training Set=================')
train_set = IEDataset(os.path.join(config.train_folder, 'train+0.txt'), max_length=config.max_length, use_type=config.use_type2)
print('==============Prepare Dev Set=================')
dev_set = IEDataset(os.path.join(config.dev_folder, 'dev+0.txt'), max_length=config.max_length, use_type=config.use_type2)
print('==============Prepare Test Set=================')
...
vocabs = update_vocabs(vocabs, [train_set, dev_set, test_set])
print(vocabs)
...
# initialize the model
adapter = Adapter(config, vocabs, output_space)
...
param_groups = [
    {
        'params': [p for n, p in adapter.named_parameters() if n.startswith('bert')],
        'lr': config.bert_learning_rate, 'weight_decay': config.bert_weight_decay
    },
    {
        'params': [p for n, p in adapter.named_parameters() if not n.startswith('bert')],
        'lr': config.learning_rate, 'weight_decay': config.weight_decay
    }
]
optimizer = AdamW(params=param_groups)
...
# model state
state = dict(model=adapter.state_dict(),
             config=config.to_dict(),
             vocabs=vocabs)
...
best_detail = None
current_step = 0
best_role_model = os.path.join(output_dir, 'best.adapter.mdl')
model.eval()
print('================Start Training================')
for epoch in range(config.max_epoch):
    print('Epoch: {}'.format(epoch))
    ...
    for batch_idx, batch in enumerate(DataLoader(
        ...
        loss = adapter(batch, prior_logit, logger, 'train_adapt', current_step)
        current_step += 1
        loss = loss * (1 / config.accumulate_step)
        loss.backward()
        ...
        if (batch_idx + 1) % config.accumulate_step == 0:
            progress.update(1)
            torch.nn.utils.clip_grad_norm_(
                adapter.parameters(), config.grad_clipping)
            optimizer.step()
            schedule.step()
            optimizer.zero_grad()
    progress.close()
    ...
    with open(log_file, 'a', encoding='utf-8') as w:
        w.write(result + '\n')
    # tensorboard
    logger.scalar_summary('dev_adapt/entity', f1, epoch)
