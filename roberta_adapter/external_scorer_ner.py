...
def transform_pred(pred_file_list):
    pred_json = {}
    for g in pred_file_list:
        entities = g['pred object']
        
        #if ' '.join(g['tokens']) in pred_json.keys():
        #    print(' '.join(g['tokens']))
...
parser.add_argument('-p', '--pred_path', required=True, help='The prediction file.')
